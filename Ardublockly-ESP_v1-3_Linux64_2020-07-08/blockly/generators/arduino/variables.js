...
Blockly.Arduino['variables_define'] = function(block) {
  var var_category = block.getFieldValue('var_category');
  var varName = Blockly.Arduino.variableDB_.getName(block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var varType = Blockly.Arduino.getArduinoType_(Blockly.Types[block.getFieldValue('VARIABLE_SETTYPE_TYPE')]);
  var value = Blockly.Arduino.valueToCode(block, 'value', Blockly.Arduino.ORDER_ATOMIC);
  
  var code = '';
  var variable_definition = varType + " " + varName + " = " + value + ";";
  
  if(var_category == "local"){
	  code = variable_definition + "\n";	  
  }
  else if(var_category == "global"){
	  Blockly.Arduino.addVariable(varName, variable_definition, true);
  }
...
Blockly.Arduino['eeprom_write'] = function(block) {
  ...
  var setupCode_eeprom_begin = "EEPROM.begin(512);";
  ...
  function_eeprom_write += "if(strlen > 512) {\n    ";
  ...
  function_eeprom_write += "if((char)bytes_of_string[z] != (char)EEPROM.read(z)) //if the correct char is already in EEPROM - NO writing to avoid unnecessary Hardware wastage\n      ";
