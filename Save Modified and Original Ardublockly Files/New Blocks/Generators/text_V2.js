...
Blockly.Arduino['text_compare'] = function(block) {
  var string_a = Blockly.Arduino.valueToCode(block, 'string_a', Blockly.Arduino.ORDER_ATOMIC);
  var string_b = Blockly.Arduino.valueToCode(block, 'string_b', Blockly.Arduino.ORDER_ATOMIC);
  var comparison = block.getFieldValue('comparison');

  //if input value is a string variable - compare it to a constant char array
  var cmp_string_a = (string_a.indexOf("\"") != 0) ? string_a + ".c_str()" : string_a;
  var cmp_string_b = (string_b.indexOf("\"") != 0) ? string_b + ".c_str()" : string_b;
  
  var code = "strcmp(" + cmp_string_a + ", " + cmp_string_b + ") " + comparison;
...
Blockly.Arduino['text_cut_delimiter'] = function(block) {
  ...
  var code = "cut_text(" + string.replace("'", "\"").replace("'", "\"") + ", " + delimiter.replace("'", "\"").replace("'", "\"") + ")";
