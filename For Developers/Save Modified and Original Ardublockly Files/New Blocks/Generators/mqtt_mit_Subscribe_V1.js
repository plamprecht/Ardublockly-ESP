...
Blockly.Arduino['mqtt_subscribe'] = function(block) {
  ...
  var function_mqtt_callback = "void mqtt_callback_"+ mqtt_variable_message + "(char* topic, byte* message, unsigned int length) {\n  ";
  function_mqtt_callback += "for (int i = 0; i < length; i++)\n    ";
  function_mqtt_callback += mqtt_variable_message + " += (char)message[i];\n}";
  var setupCode_mqtt_SetCallback = "client.setCallback(mqtt_callback_" + mqtt_variable_message + ");\n";
  var code = "client.subscribe(String(" + mqtt_topic + ").c_str());\n";
  
  Blockly.Arduino.addVariable("mqtt_callback_function", function_mqtt_callback, true);
